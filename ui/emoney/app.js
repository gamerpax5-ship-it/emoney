...
function bankAllocationForm(o){
 const banks=state.methods.filter(m=>m.type==='BANK'&&m.enabled);const expected=Number(o.inrAmount||0);
 ...
 const update=()=>{let total=inputs.reduce((sum,input)=>sum+Math.max(0,Number(input.value||0)),0),remaining=Math.max(0,expected-total);$('#distributedInr').textContent=`${money(total)} / ${money(expected)}`;$('#remainingInr').textContent=money(remaining);const valid=total===expected&&total>=0;if(error)error.textContent=total>expected?'Distribution cannot exceed the expected INR amount.':total<expected?'Assign the full expected INR before saving.':'';if(save)save.disabled=!valid;return valid};
 inputs.forEach(input=>input.oninput=update);update();
 $('#allocationForm').onsubmit=async e=>{e.preventDefault();if(!update())return;const allocations=inputs.filter(input=>Number(input.value)>0).map(input=>({payoutMethodId:input.dataset.alloc,inrAmount:Number(input.value)}));try{const result=await api(`/orders/${encodeURIComponent(o.id)}/bank-allocations`,{method:'PATCH',body:JSON.stringify({allocations})});state.orders=state.orders.map(item=>item.id===o.id?(result.order||item):item);closeModal();openOrder(o.id);toast('Bank distribution saved.')}catch(err){if(error)error.textContent=err.message;}};
}
...
function renderOverview(){
 ...
 <div class="card rates-card"><div class="rates-head"><strong>Live payout rates</strong><span class="rate-state"><i class="inline-dot"></i>Current rate</span></div><div class="rate-grid"><div class="rate-card"><span class="rate-card-icon">${uiIcon('send')}</span><div><b>UPI</b><strong>${money(state.rates?.rates?.upi)}</strong><small>Minimum ${num(state.rates?.limits?.upiMinUsdt)} USDT</small></div></div><div class="rate-card"><span class="rate-card-icon bank">${uiIcon('bank')}</span><div><b>Bank</b><strong>${money(state.rates?.rates?.bank)}</strong><small>Minimum ${num(state.rates?.limits?.bankMinUsdt)} USDT</small></div></div></div><button class="rates-refresh" data-action="rates">Refresh ${ico('refresh')}</button></div>
...
function renderRewards(){
 ...
 <div class="section-head campaign-title"><h3>Live reward campaigns</h3><button class="btn-text" data-action="campaigns">View All ${uiIcon('chevron')}</button></div><div class="campaign-grid live-campaign-grid">${state.campaigns.flatMap(c=>(c.tasks||[]).filter(t=>t.enabled).map(t=>{const claim=t.claim,eligible=canClaimTask(t);return `<article class="promo-card ${eligible?'green':''}"><div class="campaign-kicker">${esc(c.title||'Campaign')}</div><h4>${esc(t.title||'Reward task')}</h4><strong>${num(t.rewardAmount)} USDT</strong><small>${esc(t.description||'Complete the eligible task to earn this reward.')}</small><span class="reward-status">${claim?esc(claim.status==='credited'?'Claimed':claim.status):eligible?'Ready to claim':'In progress'}</span><button class="btn btn-secondary btn-sm" data-claim="${esc(t.id)}" data-campaign="${esc(t.campaignId||c.id)}" ${eligible?'':'disabled'}>${claim?esc(claim.status==='credited'?'Claimed':claim.status):eligible?(String(t.claimType).toUpperCase()==='MANUAL'?'Submit Claim':'Claim Reward'):'In Progress'}</button></article>`;})).join('')||empty('No live rewards are available right now.')}</div>
...
function showAbout() {
  modal('eMoney','Mobile application · version 1.1.2',`<div class="list"><div class="list-item"><b>Your account, in one place</b><small>Sell USDT, manage payout destinations, follow orders and access eligible rewards.</small></div><div class="list-item"><b>Deposits and payouts</b><small>TRON verification, rates, quotes, order transitions and payout decisions are handled by the existing server.</small></div><div class="notice">Never share your password, authenticator code, recovery codes or wallet private keys with another person.</div></div>`);
}
