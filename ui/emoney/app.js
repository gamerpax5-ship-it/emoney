...
function mobileOrderRow(o){return `<button class="mobile-order" data-order="${esc(o.id)}"><span class="rate-icon ${o.payoutType==='BANK'?'bank':''}">${ico(o.payoutType==='BANK'?'bank':'upi')}</span><span class="mobile-order-copy"><b>${o.payoutType==='BANK'?'Bank transfer':'UPI payout'}</b><small>${num(o.usdtAmount)} USDT · ${shortDate(o.createdAt)}</small></span><span class="mobile-order-money"><b>${money(o.inrAmount)}</b>${pillStatus(o.status)}</span></button>`}
...
async function createSellOrder() {
  ...
  try {
    if(!state._pendingOrder) {
      ...
      if(!enabled.length)throw new Error('Add and enable a payout method first.');
      if(state.rates?.channels?.[state.sellType.toLowerCase()]===false)throw new Error('This payout channel is currently unavailable.');
    ...
    if(epoch===refreshVersion){state._creating=false;for(const node of $$('#page-sell [data-type],#sellAmount,#maxBtn,#page-sell [data-amount]'))node.disabled=!!state._pendingOrder;const b=$('#createSell');if(b){b.disabled=false;b.innerHTML=(state._pendingOrder?'Retry order request':'Create Sell Order')+' '+uiIcon('arrow');}}
  }
}

function openOrder(id){
 const o=state.orders.find(x=>x.id===id);if(!o)return;const refs=o.payout?.references||o.payout?.allocations||[];
 modal('Transaction details',esc(o.id),`<div class="detail-head"><div class="detail-total">${money(o.inrAmount)}<small>${num(o.usdtAmount)} USDT · ${esc(o.payoutType)} payout</small></div>${pillStatus(o.status)}</div><div class="detail-grid"><div class="list-item"><span>Confirmations</span><b>${Number(o.confirmations||0)} / ${Number(o.requiredConfirmations||19)}</b></div><div class="list-item"><span>USDT received</span><b>${o.receivedUsdt==null?'Awaiting deposit':num(o.receivedUsdt)}</b></div></div><div class="list"><div class="list-item"><b>${('Assigned TRON address')}</b><small>${esc(o.depositAddress||'Not assigned')}</small></div><div class="list-item"><b>Transaction hash</b><small>${esc(o.txId||'Not detected yet')}</small></div></div>${o.review?`<div class="notice" style="margin-top:12px">${esc(o.review.reason||'This order needs manual review.')}</div>`:''}<div class="section-head" style="margin-top:23px"><h3>Order timeline</h3></div><div class="timeline">${(o.timeline||[]).map(t=>`<div class="tl"><span class="tl-dot">✓</span><div><b>${esc(t.status)}</b><small>${dt(t.at)}</small></div></div>`).join('')||empty('No events yet.')}</div>${refs.length?`<div class="section-head" style="margin-top:25px"><h3>Payout references</h3></div><div class="list">${refs.map(r=>`<div class="list-item"><b>${esc(r.mode)} · ${esc(r.reference)}</b><small>${money(r.inrAmount??Number(r.inrPaise||0)/100)}</small></div>`).join('')}</div>`:''}${o.payoutType==='BANK'&&o.flexibleBankAllocation&&!['Expired','Completed','Rejected','Failed'].includes(o.status)?`<button class="btn btn-secondary full" id="bankDistribution" style="margin-top:20px">Manage bank distribution</button>`:''}${(!['Completed','Failed','Rejected'].includes(o.status))?`<button class="btn btn-ghost full" id="checkTx" style="margin-top:12px">Check a transaction hash</button>`:''}${activeStatuses.includes(o.status)?`<button class="btn btn-secondary full" id="viewDeposit" style="margin-top:12px">Open deposit</button>`:''}`);
...
async function saveMethod(e,type){
 ...
 closeModal();await refreshAll(true);renderAll();openPayoutMethods();toast('Payout method saved.');
 }catch(error){$('#methodError').textContent=error.message}finally{b.disabled=false}
}
async function toggleMethod(id){try{const m=state.methods.find(x=>x.id===id);await api(`/payout-methods/${encodeURIComponent(id)}/status`,{method:'PATCH',body:JSON.stringify({enabled:!m.enabled})});await refreshAll(true);openPayoutMethods();toast('Payout method updated')}catch(e){toast(e.message,true)}}
async function deleteMethod(id){try{await api(`/payout-methods/${encodeURIComponent(id)}`,{method:'DELETE'});await refreshAll(true);openPayoutMethods();toast('Payout method deleted')}catch(e){toast(e.message,true)}}
...
function bankAllocationForm(o){
 const banks=state.methods.filter(m=>m.type==='BANK'&&m.enabled);modal('Bank distribution','Allocate your INR payout to your enabled bank accounts.',`<div class="notice info">Order obligation: ${money(o.inrAmount)}. Your backend validates available limits and amounts already paid.</div><form id="allocationForm" style="margin-top:18px">${banks.map(m=>`<div class="field"><label for="alloc-${esc(m.id)}">${esc(m.bankName||m.label)} · ••${esc(String(m.accountNumber||'').slice(-4))}</label><input class="control" id="alloc-${esc(m.id)}" type="number" min="0" step="0.01" inputmode="decimal" data-alloc="${esc(m.id)}" value="${Number((o.allocations||[]).find(a=>a.payoutMethodId===m.id)?.inrPaise||0)/100}"></div>`).join('')}<div class="auth-message" id="allocError"></div><div class="modal-actions"><button type="button" class="btn btn-ghost" id="backOrder">Back</button><button class="btn btn-primary" type="submit" ${banks.length?'':'disabled'}>Save distribution</button></div></form>`);
 $('#backOrder').onclick=()=>openOrder(o.id);$('#allocationForm').onsubmit=async e=>{e.preventDefault();try{const allocations=$$('[data-alloc]').map(i=>({payoutMethodId:i.dataset.alloc,inrAmount:Number(i.value)})).filter(a=>a.inrAmount>0);if(allocations.reduce((s,a)=>s+a.inrAmount,0)>Number(o.inrAmount)+.0001)throw new Error('Distribution cannot exceed the order payout.');{const r=await api(`/orders/${encodeURIComponent(o.id)}/bank-allocations`,{method:'PATCH',body:JSON.stringify({allocations})});state.orders=state.orders.map(x=>x.id===o.id?r.order:x)}toast('Bank distribution saved');openOrder(o.id)}catch(error){$('#allocError').textContent=error.message}};
}
...
function renderNav(){
 ...
 $('#mobileNav').innerHTML=navs.map(([id,label,i])=>`<button data-page="${id}" class="${state.page===id?'active':''}" ${state.page===id?'aria-current="page"':''}>${uiIcon(i)}<span>${label}</span></button>`).join('');
...
function renderOverview(){
 ...
 <div class="money-summary dark-card"><div class="money-left"><div class="eyebrow">Total Paid Out <button id="hideAmounts" aria-label="${state._hideBalance?'Show':'Hide'} amounts">${ico('eye')}</button></div><div class="large-currency">${state._hideBalance?'₹••,•••':money(payout)}</div><div class="volume-pill"><span class="mini-token">₮</span>${state._hideBalance?'••••':num(volume)} USDT <span style="color:#c8dacd"> · completed</span></div></div><button class="money-right" data-jump="rewards">${uiIcon('bars')}<span>Total Rewards</span><strong>${num(state.rewards?.balance)} <small>USDT</small></strong><em>Keep going! ${uiIcon('chevron')}</em></button></div>
 <div class="card rates-card">${tokenIcon()}<div class="rates-copy"><div class="rates-head"><strong>USDT to INR</strong><span class="rate-state"><i class="inline-dot"></i>${('Current rate')}</span></div><div class="rate-amount">${money(state.rates?.rates?.upi)}</div><div class="rate-caption">1 USDT = ${money(state.rates?.rates?.upi)} · UPI</div></div><button class="rates-refresh" data-action="rates">${('Refresh')} ${ico('refresh')}</button>${('')}</div>
...
function renderSell(){
 ...
 <div class="payout-tabs" role="tablist"><button data-type="UPI" ${orderLocked?'disabled':''} role="tab" aria-selected="${type==='UPI'}" class="${type==='UPI'?'active':''}">${uiIcon('send')}<span><b>UPI Payout</b><small>Receive INR in your UPI</small></span></button><button data-type="BANK" ${orderLocked?'disabled':''} role="tab" aria-selected="${type==='BANK'}" class="${type==='BANK'?'active':''}">${uiIcon('bank')}<span><b>Bank Payout</b><small>Direct to your bank account</small></span></button></div>
 <div class="card amount-panel"><div class="amount-inner"><label class="amount-label" for="sellAmount">You Sell</label><div class="amount-entry"><input id="sellAmount" ${orderLocked?'disabled':''} inputmode="decimal" autocomplete="off" placeholder="0" value="${esc(state._amount||'')}" aria-label="USDT amount"><span class="token-select">${tokenIcon()}USDT ${uiIcon('chevron')}</span></div></div><div class="amount-chips">${choices.map(n=>`<button data-amount="${n}" ${orderLocked?'disabled':''} class="${amount===n?'active':''}">${num(n)}</button>`).join('')}<button id="maxBtn" ${orderLocked?'disabled':''}>MAX</button></div><div class="amount-limits"><span>Min ${num(min)} USDT</span><span>Max ${num(state.rates?.limits?.globalMaxUsdt)} USDT</span></div></div>
 ...
 <button class="card destination" data-action="payouts"><span class="destination-symbol">${type==='UPI'?'<svg viewBox="0 0 32 36" fill="none" aria-hidden="true"><path fill="#15925a" d="m17 2 12 18L9 34Z"/><path fill="#ef8d19" d="m8 3 12 18L0 34Z"/></svg>':uiIcon('bank')}</span><span class="destination-copy"><b>Payout Account (${type})</b><strong>${esc(methodName||'Add a payout method')}</strong><small>${esc(method?.holderName||'Enable a UPI ID or bank account')}</small></span><span class="destination-action">Manage ${uiIcon('chevron')}</span></button>
 ...
 $('#createSell').onclick=createSellOrder;bindActions();if(o)renderQr(o);
}
...
function renderRewards(){
 ...
 <div class="section-head campaign-title"><h3>Reward Campaigns</h3><button class="btn-text" data-action="campaigns">View All ${uiIcon('chevron')}</button></div>
 <div class="campaign-grid"><button class="promo-card green" data-action="referrals"><h4>Refer & Earn</h4><strong>More together</strong><small>Invite friends.<br>Earn eligible rewards.</small>${art('promoPeople','','') }<span class="round-arrow">${uiIcon('chevron')}</span></button><button class="promo-card" data-action="campaigns"><h4>Transact & Earn</h4><strong>Extra Rewards</strong><small>Complete tasks.<br>Track every reward.</small>${art('promoGift','','')}<span class="round-arrow">${uiIcon('chevron')}</span></button><button class="promo-card mint" data-action="campaigns"><h4>Special Offers</h4><strong>Just for you</strong><small>Explore available<br>reward campaigns.</small>${art('promoMegaphone','','')}<span class="round-arrow">${uiIcon('chevron')}</span></button></div>
 ...
 $('#spinBtn').onclick=spinWheel;bindJumps();bindActions();
}
...
function renderProfile(){
 ...
 <div class="card settings-group"><h3>Payments & Payouts</h3>${settingRow('wallet','Payout Methods','Manage your bank accounts and payout preferences','payouts')}</div>
 ...
 <div class="card settings-group"><h3>Preferences</h3>${settingRow('bell','Notifications','Manage alerts and updates','notification-settings')}${settingRow('globe','Language','Choose your preferred language','language',localLanguage==='hi'?'हिन्दी':'English')}</div>
...
function showCampaigns() {
  const tasks=state.campaigns.flatMap(c=>(c.tasks||[]).filter(t=>t.enabled).map(t=>({...t,campaignTitle:c.title,campaignId:t.campaignId||c.id})));
...
function applyNavLanguage(){if(localLanguage!=='hi')return;const words={overview:'होम',sell:'बेचें',orders:'ऑर्डर',rewards:'रिवार्ड',profile:'प्रोफाइल'};$$('#mobileNav button').forEach(b=>{b.querySelector('span').textContent=words[b.dataset.page]})}
...
function rateDetails() {
  modal('USDT payout rates','Current account rates from the server.',`<div class="list"><div class="list-item"><b>UPI · ${state.rates?money(state.rates.rates.upi):'—'} / USDT</b><small>Minimum ${num(state.rates?.limits?.upiMinUsdt)} USDT</small></div><div class="list-item"><b>Bank · ${state.rates?money(state.rates.rates.bank):'—'} / USDT</b><small>Minimum ${num(state.rates?.limits?.bankMinUsdt)} USDT</small></div><div class="notice info">The final rate and amount come from your server quote. Payout is subject to deposit verification and processing.</div></div>`);
}
function showAbout() {
  modal('eMoney','Mobile application · version 1.1.0',`<div class="list"><div class="list-item"><b>Your account, in one place</b><small>Sell USDT, manage payout destinations, follow orders and access eligible rewards.</small></div><div class="list-item"><b>Deposits and payouts</b><small>TRON verification, rates, quotes, order transitions and payout decisions are handled by the existing server.</small></div><div class="notice">Never share your password, authenticator code, recovery codes or wallet private keys with another person.</div></div>`);
}
function handleAction(a){const actions={'payouts':openPayoutMethods,'edit-profile':openProfileEdit,'security':openSecurity,'sessions':openSessions,'support':openSupport,'notifications':openNotifications,'toggle-notifications':toggleNotifications,'logout':confirmLogout,'account':()=>{closeModal();go('profile');window.scrollTo(0,0)},'search':openSearch,'about':showAbout,'export':exportOrders,'reward-history':openRewardHistory,'referrals':openReferrals,'campaigns':showCampaigns,'share-referral':shareReferral,'notification-settings':notificationSettings,'language':languageSettings,'auth-help':authHelp,'forgot':forgotPassword,'rates':rateDetails};actions[a]?.()}
