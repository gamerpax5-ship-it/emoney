...
const downloadPrelude = `    if (isDigiRupeeHost && pathname === '/download/digirupee.apk') {\n      const pendingReferralCode = String(url.searchParams.get('ref') || '').trim().toUpperCase();\n      if (pendingReferralCode) {\n        const referrer = db.digirupee.users.find(item => String(item.referralCode || '').toUpperCase() === pendingReferralCode);\n        if (referrer) {\n          const now = Date.now();\n          const key = digiSessionIpHash(req);\n          db.digirupee.pendingReferralInstalls = (Array.isArray(db.digirupee.pendingReferralInstalls) ? db.digirupee.pendingReferralInstalls : [])\n            .filter(item => Number(item.expiresAt || 0) > now && item.key !== key);\n          db.digirupee.pendingReferralInstalls.push({\n            key,\n            referralCode: referrer.referralCode,\n            createdAt: now,\n            expiresAt: now + 6 * 60 * 60 * 1000\n          });\n          await persist();\n        }\n      }\n      const candidates = [`;
const downloadPreludeForAlias = downloadPrelude.replace("pathname === '/download/digirupee.apk'", "['/download/digirupee.apk', '/download/emoney.apk'].includes(pathname)");
source = source.replace(downloadNeedle, downloadPreludeForAlias);
