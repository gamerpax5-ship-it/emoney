...
let source = (await readFile(runtimePath, 'utf8')).replace(/\r\n/g, '\n');
...
if (!source.includes("path === '/referral-install/claim'")) source = source.replace(apiNeedle, apiNeedle + claimRoute);
...
if (!source.includes("const pendingReferralCode = String(url.searchParams.get('ref')")) source = source.replace(downloadNeedle, downloadNeedle + downloadPrelude);
